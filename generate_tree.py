...
def fetch_data_and_generate():
    ...
    if not df.empty:
        ...
        sire_sire_col = next((c for c in df.columns if 'Sire 美系父親名' in c or '祖父' in c), None)
        sire_dam_col = next((c for c in df.columns if 'Dam Name 美系母親名' in c and '外' not in c or '祖母' in c), None)
